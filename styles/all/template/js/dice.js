...
(function($) { // Avoid conflicts with other libraries
	...
	/**
	 * BEGIN roll all dice code
	 */
	//const regex = /\[roll\](?<expr>[a-zA-Z0-9+*\-\/=><!\(\) ]+)\[\/roll\]/gi;
	const regex = /\[roll\](?<expr>[dDlLfFpP0-9.+*\-\/%=><!\(\) ]+)\[\/roll\]/gi;

	function getRollsToSubmit(text) {
	...
	function handleRoll(text, rollObjects, finishFn) {
		if (rollObjects.length === 0) {
			return finishFn(text);
		}
	
		const rollObj = rollObjects.shift();
		const params = {notation: rollObj.groups.expr};
		const submitUrl = $('#dice_roll_all').attr('data-url');
		...
		let request = $.ajax({
			...
			data: params,
			cache: false,
			success: handleResponse(rollObj, text, rollObjects, finishFn),
			error: handleResponse(rollObj, text, rollObjects, finishFn),
		});
	}
	
	function handleResponse(rollObj, text, rollObjects, finishFn) {
		return (data) => {
			if (data.ROLL_SUCCESS) {
				const length = rollObj[0].length;
				const beginning = text.substring(0, rollObj.index);
				const end = text.substring(rollObj.index + length);
				const newRoll = `[roll=${data.ROLL_DATA.id}]${data.ROLL_DATA.notation}[/roll]`;
				const newText = `${beginning}${newRoll}${end}`;

				// We add the roll to the table
				dice.setRollRow(data.ROLL_DATA);
	
				handleRoll(newText, rollObjects, finishFn);
			} else {
				handleRoll(text, rollObjects, finishFn);
			}
		};
	}
	/**
	 * END roll all dice code
	 */
	...
	let $rollAllButton = $('#dice_roll_all');
	...
	$(function() {
		// Hide roll all if not edit mode
		...
		// Bind the 'Create all rolls'-button
		$rollAllButton.on('click', function(event) {
			let $loadingIndicator = phpbb.loadingIndicator();

			let text = $message.val(),
				rollObjects = getRollsToSubmit(text);

			// After the request (success or error) remove the loading indicator
			const finishFn = function(text) {
				$message.val(text);

				phpbb.clearLoadingTimeout();

				// Show the response message
				phpbb.alert("All Rolls Finished", "You can save the post now.");
			...
			// Call the 'Create all rolls'-function
			handleRoll(text, rollObjects, finishFn);
